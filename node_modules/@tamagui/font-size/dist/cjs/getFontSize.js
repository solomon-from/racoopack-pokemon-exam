...
const getFontSize = (inSize, opts) => {
  ...
  return token ? (0, import_core.getConfig)().fontsParsed[opts?.font || "$body"].size[token] : inSize;
}, getFontSizeToken = (inSize, opts) => {
  if (typeof inSize == "number")
    return null;
  const relativeSize = opts?.relativeSize || 0, fontSize = (0, import_core.getConfig)().fontsParsed[opts?.font || "$body"].size, size = (inSize === "$true" && !("$true" in fontSize) ? "$4" : inSize) ?? ("$true" in fontSize ? "$true" : "$4"), sizeTokens = Object.keys(fontSize);
